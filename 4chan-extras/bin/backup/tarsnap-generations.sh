...
for dir in $(cat $PATHS) ; do
	/usr/local/bin/tarsnap -c -f $NOW-$BK_TYPE-$(hostname -s)-$(echo $dir) --one-file-system -C / $dir
	if [ $? = 0 ] ; then
	    if [ $QUIET != "1" ] ; then
...
archive_list=$(/usr/local/bin/tarsnap --list-archives)
		...
		case "$backup" in
			 "$HOURLY_DELETE_TIME-$BK_TYPE"* ) 	
					case "$backup" in   #this case added to make sure the script doesn't delete the backup it just took. Case: '-h x' and backup takes > x hours. 
						...
						* )  /usr/local/bin/tarsnap -d -f $backup
							if [ $? = 0 ] ; then
							    if [ $QUIET != "1" ] ; then
