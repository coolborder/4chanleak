...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="/css/stafflog.css?11">
  <link rel="shortcut icon" href="/favicon.ico" type="image/x-icon">
  <script type="text/javascript" src="/js/admincore.js?30"></script>
  <script type="text/javascript" src="/js/helpers.js"></script>
  <script type="text/javascript" src="/js/stafflog.js?8"></script>
</head>
<body>
<header>
  <h1 id="title">Staff Log</h1><div id="cfg-btn"><span>&hellip;</span><div><label><input data-cmd="toggle-dt" id="cfg-cb-dt" type="checkbox" autocomplete="off">Dark Theme</label></div></div>
</header>
...
<ul>
  ...
  <li><span class="form-label">Thread No.</span><input value="<?php if (isset($this->url_params['thread'])) echo (int)$this->url_params['thread'] ?>" type="text" id="filter-thread"></li>-->
  ...
  <li><label><span class="form-label" data-tip="Thread OPs only">OPs</span><input<?php if (isset($this->url_params['ops'])) echo ' checked' ?> type="checkbox" id="filter-ops"></label></li>
  <li><label><span data-tip="Skip automatic deletions" class="form-label">Manual</span><input<?php if (isset($this->url_params['manual'])) echo ' checked' ?> type="checkbox" id="filter-manual"></label></li>
...
<?php foreach ($this->entries as $entry): $action = $this->formatAction($entry); ?>
  <tr data-id="<?php echo $entry['id'] ?>">
    ...
    <?php endif ?><?php if ($entry['tool']): ?>
      <div class="tool"><?php echo $entry['tool'] ?></div>
    <?php endif ?>
    </td>
    <td class="col-board">/<?php echo $entry['board'] ?>/</td>
    <td class="col-post"><span class="post-no">No.<span class="as-iblk"><?php echo $entry['postno'] ?></span> <?php if (isset($entry['link'])): ?><a data-tip="Archive Link" target="_blank" href="https://www.4chan.org/derefer?url=<?php echo rawurlencode($entry['link']) ?>">→</a><?php endif ?></span><?php if (isset($entry['resto']) && !$entry['resto']): ?><span class="post-isop">OP</span><?php endif ?><div><span class="name"><?php echo $entry['name'] ?><?php if ($entry['tripcode']): ?><span class="tripcode">!<?php echo $entry['tripcode'] ?></span><?php endif ?></span><?php if ($entry['filename']): ?><span class="filename"><?php echo $entry['filename'] ?></span><?php endif ?></div><?php if ($entry['sub']): ?><div class="subject"><?php echo $entry['sub'] ?></div><?php endif ?><div class="comment"><?php echo $entry['com'] ?></div></td>
