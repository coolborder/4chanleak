...
class APP {
  ...
  private function get_log() {
    ...
    if (!$use_actions_log) {
      // Thread (too slow, no indexes)
      /*
      if (isset($_GET['thread']) && $_GET['thread']) {
        $url_params['thread'] = (int)$_GET['thread'];
        $clauses[] = 'resto = ' . (int)$_GET['thread'];
      }
      // OPs only
      else */if (isset($_GET['ops']) && $_GET['ops']) {
        $url_params['ops'] = 1;
        $clauses[] = 'resto = 0';
      }
    }
    ...
    $query = <<<SQL
SELECT SQL_NO_CACHE id, postno, board, name, sub, com, filename, admin, $columns,
...
SQL;
    ...
    while ($row = mysql_fetch_assoc($result)) {
      ...
      $thread_id = (isset($row['resto']) && $row['resto']) ? $row['resto'] : $row['postno'];
      
      $link = return_archive_link($row['board'], $row['postno'], false, true, $thread_id);
